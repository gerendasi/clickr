var React = require('react'),
    ...
    componentDidMount: function() {
      ...
      particle.on('login', function(err, body) {
        ...
        var deviceId = DataStore.get('particleID');

        if (deviceId) {
          particle.getDevice(deviceId, function(err, device) {
            if (err) {
              console.log('Particle Core error came through: ', err);
            } else {
          ...
          console.log('No Particle device recognised?');
        }
      });
